...
def append_jobs(conn: sqlite3.Connection, jobs: list[Job]) -> int:
    for job in jobs:
        conn.execute(
            "INSERT INTO jobs (company, title, salary, requirements, apply_link, source, location) "
            "VALUES (?, ?, ?, ?, ?, ?, ?)",
            (job.company, job.title, job.salary_raw, job.requirements, job.apply_link, job.source, job.location),
        )
    conn.commit()
    return len(jobs)
